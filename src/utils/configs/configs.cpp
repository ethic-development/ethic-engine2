...
void ethic::utils::configs::save(const char *name)
{
    json config;
    ...
    saveFile(appdata_path() + "\\ethic\\" + "\\configs\\" + static_cast<std::string>(name) + ".cfg", config.dump());
}

void ethic::utils::configs::load(const char *name)
{
    std::string configJson = "";
    loadFile(appdata_path() + "\\ethic\\" + "\\configs\\" + static_cast<std::string>(name) + ".cfg", configJson);
