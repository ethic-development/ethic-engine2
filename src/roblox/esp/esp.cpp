...
float calc_dist_3d(ethic::roblox::vector3_t first, ethic::roblox::vector3_t sec)
{
	return sqrt((first.x - sec.x) * (first.x - sec.x) + (first.y - sec.y) * (first.y - sec.y) + (first.z - sec.z) * (first.z - sec.z));
}

ethic::roblox::vector3_t vector_sub(ethic::roblox::vector3_t one, ethic::roblox::vector3_t two)
{
	return { one.x - two.x, one.y - two.y, one.z - two.z };
}

float vector3_mag(ethic::roblox::vector3_t vector)
{
...
void ethic::roblox::hook_esp()
{
    ...
    ethic::roblox::vector3_t camerapos = globals::visualengine.get_camera_pos();

    if (globals::fov_on)
    {
        draw->AddCircle(ImVec2(cursor_pos.x, cursor_pos.y), globals::fov, ImColor(75, 95, 85, 255));
    }
    ...
    if (globals::esp)
    {
        for (auto player : players.children())
        {
            ...
            auto head_position = ethic::roblox::world_to_screen(vector_sub(head_position_3d, { 0, 2, 0 }));

            auto hrp_pos = hrp.get_part_pos();

            auto leg_pos = ethic::roblox::world_to_screen(vector_sub(head.get_part_pos(), { 0, 5, 0 }));
            if (leg_pos.x == -1)
                continue;
            ...
            if (globals::head_circle)
            {
                DrawHeadCircle(head_position.x, head_position.y - 50.f, width / 3.5f, ImColor(1.f, 0.f, 0.f), 1.5f);
            }
            ...
            if (globals::name_esp)
            {
                ...
                draw->AddText(ImVec2(head_position.x, head_position.y + 50), ImColor(0, 0, 0), playerName.c_str());
                draw->AddText(ImVec2(head_position.x, head_position.y + 50), ImColor(0, 0, 0), playerName.c_str());
                draw->AddText(ImVec2(head_position.x, head_position.y + 50), ImColor(0, 0, 0), playerName.c_str());
                draw->AddText(ImVec2(head_position.x, head_position.y + 50), ImColor(0, 0, 0), playerName.c_str());

                draw->AddText(ImVec2(head_position.x, head_position.y + 50), ImColor(255, 255, 255), playerName.c_str());
            }
            ...
            if (globals::box)
            {
                ...
                ImColor fill_color(178, 0, 255, 51);
                ...
                case 0:
                    ...
                    draw->AddRect(
                        ImVec2(leg_pos.x - (width / 1.3f), leg_pos.y),
                        ImVec2(head_position.x + width, head_position.y + height),
                        white_outline_color
                    );

                    // Add first black outline
                    draw->AddRect(
                        ImVec2(leg_pos.x - (width / 1.3f) - outline_offset - 1.5, leg_pos.y - outline_offset - 1.5),
                        ImVec2(head_position.x + width + outline_offset + 1.5, head_position.y + height + outline_offset + 1.5),
                        black_outline_color
                    );

                    // Add second black outline
                    draw->AddRect(
                        ImVec2(leg_pos.x - (width / 1.3f) - outline_offset + 1.5, leg_pos.y - outline_offset + 1.5),
                        ImVec2(head_position.x + width + outline_offset - 1.5, head_position.y + height + outline_offset - 1.5),
                        black_outline_color
                ...
                case 2:
                    draw->AddRectFilled(
                        ...
                        ImVec2(head_position.x + width + outline_offset, head_position.y + height + outline_offset),
                        fill_color
                    );
                        draw->AddRect(
                            ImVec2(leg_pos.x - (width / 1.3f), leg_pos.y),
