...
float calc_dist(ethic::roblox::vector2_t first, ethic::roblox::vector2_t sec)
{
	return sqrt((first.x - sec.x) * (first.x - sec.x) + (first.y - sec.y) * (first.y - sec.y));
}

ethic::roblox::vector2_t add_vector_2(ethic::roblox::vector2_t first, ethic::roblox::vector2_t sec)
{
	return
	{
		first.x + sec.x,
...
ethic::roblox::vector3_t add_vector_3(ethic::roblox::vector3_t first, ethic::roblox::vector3_t sec)
{
	return
	{
		first.x + sec.x,
...
ethic::roblox::vector3_t div_vector_3(ethic::roblox::vector3_t first, ethic::roblox::vector3_t sec)
{
	return
	{
		first.x / sec.x,
...
ethic::roblox::vector2_t vector_sub(ethic::roblox::vector2_t one, ethic::roblox::vector2_t two)
{
	return { one.x - two.x, one.y - two.y };
}

ethic::roblox::instance_t ethic::roblox::get_closest_player_to_cursor()
{
	POINT cursor_point;
	...
	ethic::roblox::instance_t closest_player;
	...
	for (auto player : players.children())
	{
		...
		if (globals::prediction)
		{
			...
			auto velocity_vec = div_vector_3(velocity, { globals::prediction_x, globals::prediction_y});
		...
		auto head_pos = ethic::roblox::world_to_screen(head_pos_3d);
...
ethic::roblox::vector2_t ethic::roblox::get_relative_player_to_pos(vector2_t closest_player)
{
	vector2_t ret = { 0, 0 };
...
void run(ethic::roblox::instance_t player)
{
	POINT cursor_pos;
	...
	ethic::roblox::vector2_t relative = { 0, 0 };
...
ethic::roblox::instance_t saved;
bool saveda = false;

void ethic::roblox::hook_aimbot()
{
	while (true)
	{
