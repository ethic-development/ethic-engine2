...
void ethic::utils::log::debug_log(const std::uint64_t address, const char *name)
{
	printf("[+] roblox: %s: %p\n", name, address);
}

std::vector<std::filesystem::path> ethic::utils::log::get_roblox_file_logs()
{
    std::vector<std::filesystem::path> roblox_log;
    std::wstring roblox_log_path = L"C:\\Users\\" + ethic::utils::log::get_username() + L"\\AppData\\Local\\Roblox\\logs";
...
std::wstring ethic::utils::log::get_username()
{
...
std::filesystem::path ethic::utils::log::get_latest_log()
{
